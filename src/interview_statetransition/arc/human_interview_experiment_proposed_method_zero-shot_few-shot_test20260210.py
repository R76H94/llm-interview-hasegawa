...
def interviewer_llm_generate_question(state: State) -> State:
    ...
    if current_topic and current_topic in all_slots:
    ...
    try:
        ...
        if response and hasattr(response, "content"):
            ...
            try:
                ...
                selected_topic = None
                if isinstance(target_slot_dict, dict) and len(target_slot_dict) > 0:
                    selected_topic = next(iter(target_slot_dict.keys()))

                if TOPIC_MODE == "record" and selected_topic:
                    ts = new_state.get("topic_sequence") or []
                    # 重複排除したいなら↓（不要なら消してOK）
                    already = any(x.get("selected_topic") == selected_topic for x in ts)
                    if not already:
                        ts.append(
                            {
                                "step": new_state.get(
                                    "slot_generation_count", 0
                                ),  # あるいは sc["total_count"] でもOK
                                "selected_topic": selected_topic,
                                "source": "question_target_slot",
                            }
                        )
                        new_state["topic_sequence"] = ts
...
def decide_topic_from_sequence_only(st: State) -> Optional[str]:
    """
    current_topic は必ず topic_sequence から取る（pacは直接入れない）
    - replay: topic_sequence を topic_replay_index 順に消費
    - record/free: 足りなければ pac から選んで topic_sequence に追記してから消費
    """
    topic_sequence = st.get("topic_sequence") or []
    st["topic_sequence"] = topic_sequence  # 念のため存在保証

    idx = st.get("topic_replay_index", 0)
    pac = list(st.get("persona_attribute_candidates") or [])

    # 1) すでに topic_sequence にあるなら、それを使う（replay含む）
    if idx < len(topic_sequence):
        topic = topic_sequence[idx].get("selected_topic")
        if topic is None:
            st["current_topic"] = None
            return None
        st["topic_replay_index"] = idx + 1  # indexはここでだけ進める
        st["current_topic"] = topic
        # 重複防止（任意）
        if topic in pac:
            pac.remove(topic)
        st["persona_attribute_candidates"] = pac
        return topic

    # 2) topic_sequence が尽きた場合
    if TOPIC_MODE in ("record", "free"):
        if not pac:
            st["current_topic"] = None
            st["persona_attribute_candidates"] = pac
            return None

        picked = random.choice(pac)
        pac.remove(picked)

        st["topic_sequence"].append(
            {
                "step": st.get("slot_generation_count", 0) + 1,
                "selected_topic": picked,
                "slot_generation_count": st.get("slot_generation_count", 0),
            }
        )
        st["persona_attribute_candidates"] = pac

        # 追記したので同じ idx を消費
        topic = st["topic_sequence"][idx]["selected_topic"]
        st["topic_replay_index"] = idx + 1
        st["current_topic"] = topic
        return topic

    # replay で尽きたら topic なし
    st["current_topic"] = None
    return None
...
def interviewer_llm_generate_slots_2(state: State) -> State:
    ...
    topic = new_state.get("current_topic")  # ここだけを見る

    # topic が無いなら LLM呼び出しはせず保存して戻る（indexは触らない）
    if not topic:
        new_state["last_generated_slot"] = []
        now = datetime.datetime.now(jst)
        timestamp = now.strftime("%Y%m%d_%H%M%S")
...
def select_generate_slots_node(state: State) -> State:
    ...
    # 直近のインタビュー対象者の発話を取得
    last_interviewee_utterance = None
    for message in reversed(new_state["dialogue_history"]):
        if message.startswith("インタビュー対象者:"):
            last_interviewee_utterance = message
            break

    def choose_slots2_or_skip(st: State) -> None:
        topic = decide_topic_from_sequence_only(st)
        if topic is None:
            st["branch"] = "skip_to_question"
        else:
            st["branch"] = "interviewer_llm_generate_slots_2"

    def choose_slots_deep(st: State) -> None:
        st["branch"] = "interviewer_llm_generate_slots"

    # 「わかりません」なら slots_2 優先
    if last_interviewee_utterance and "わかりません" in last_interviewee_utterance:
        choose_slots2_or_skip(new_state)
        return new_state
    ...
    if SLOT_SELECTION_MODE == "llm":
        ...
        if branch == "interviewer_llm_generate_slots_2":
            choose_slots2_or_skip(new_state)
        elif branch == "interviewer_llm_generate_slots":
            choose_slots_deep(new_state)
        else:
