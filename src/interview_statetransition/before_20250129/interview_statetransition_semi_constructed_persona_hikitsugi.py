# nohup python src/interview_statetransition/interview_statetransition_semi_constructed_persona_hikitsugi.py > out/log/output_$(date +%Y%m%d_%H%M%S).log 2>&1 &
...
PERSONA_SETTINGS_PATH = (
    "/mnt/work/interview/data/hashimoto-nakano/persona_settings/kato_p.txt"
)

# 自己評価アンケート
SELF_EVALUATION_PATH = (
    "/mnt/work/interview/data/hashimoto-nakano/questionnaire/kato_q.json"
)
...
INTERVIEW_CONFIG = {
    ...
    "persona_coverage": "0",
    "slots": {
        "現在のキャリア": {"category": "キャリア", "value": None},
        "悩みや不満点": {"category": "キャリア", "value": None},
    },
}
...
def analize_persona_coverage(state: State) -> str:
    ...
    human_message = "\nペルソナの情報をどれだけ聞き出せているか(カバー率)を分析してください。\nカバー率: "
...
graph_builder.add_node(
    "interviewer_llm_generate_slots_1", interviewer_llm_generate_slots
)
graph_builder.add_node(
    "interviewer_llm_generate_slots_2", interviewer_llm_generate_slots
)
...
graph_builder.add_conditional_edges(
    ...
    has_career_topic,
    {
        True: "interviewer_llm_generate_slots_1",
        False: "interviewer_llm_idle_talk",
    },
)
graph_builder.add_edge("interviewer_llm_generate_slots_1", "interviewer_llm_fill_slots")
graph_builder.add_edge("interviewer_llm_fill_slots", "analize_persona_coverage")
graph_builder.add_conditional_edges(
    "analize_persona_coverage",
    finish_interview,
    {
        "end": "end_interview",
        "continue": "interviewer_llm_generate_slots_2",
    },
)
graph_builder.add_conditional_edges(
